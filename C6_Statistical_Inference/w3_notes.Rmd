...
When we replace $\sigma$ with $S$, it turns into a t-distribution. When $n$ increases, the distinction becomes irrelevant, but with small $n$ the distinction can be quite wide. If you use the standard normal with small sample sizes you get confidence intervals that are too narrow.
...
Looking at the sleep data that was originally used in Gosset's paper:

```{r sleep_data}
head(sleep)

sleep %>%
    ggplot(aes(as.numeric(group), extra)) +
    geom_line(aes(colour = ID)) +
    geom_point(size = 4, colour = 'lightblue', shape = 21) +
    labs(x = 'Group', y = 'Extra')

# Confidence intervals
sleep %>%
    t.test(extra ~ I(relevel(group, 2)), paired = T, data = .) %>%
    .$conf.int
```
...
sleep %>%
    group_by(group) %>%
    mutate(diff =  extra - extra)
```

## Independent Group t Confidence Intervals

- Suppose we want to compare the mean bloud pressure between two groups in a randomised trial: those who received the treatment and those who received a placebo.
...
We want to test whether the population of son's heights is equivalent to the population of father's heights - i.e. the population mean is 0.

```{r father_son, message = F}
library(UsingR)

t.test(father.son$fheight - father.son$sheight)
```

Remembering the t-statistic is the ratio of the departure of the estimated value of a parameter from its hypothesised value to the standard error.

$$ t_{\hat{\beta}} = \frac{ \hat{\beta} - \beta_0 }{ SE(\hat{\beta} ) } $$

Here our $\beta_0 = 0$.

In our case we have a t-statistic of 11, which is large, so we reject the null hypothesis.

## Two Group Intervals

We know how to do two group T tests: we have covered independent group T intervals. The rejection rules are the same.
...
## Poisson Example
    ...
    > Suppose that a hospital has an infection rate of 10 infections per 100 person/days at a risk (rate) of 0.1 during the last monitoring period. Assume that an infection rate of 0.05 is an important benchmark. Given the model, could the observed rate being larger than 0.05 be attributed to chance?
